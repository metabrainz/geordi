...
def check_type(mbid):
    req = urllib2.Request(
        'http://musicbrainz.org/ws/js/entity/{mbid}'.format(mbid=mbid)
    )
...
def register_match(index, item, itemtype, matchtype, mbids,
                   auto=False, user=None, ip=False):
    if matchtype != 'unmatch':
        if len(mbids) < 1:
            response = Response(json.dumps({
                'code': 400,
                'error': 'You must provide at least one MBID for a match.'
        ...
        try:
            ...
            response = Response(json.dumps({
                'code': 400,
                'error': 'A provided MBID is ill-formed'
            }), 400, mimetype="application/json")
            response.headers.add('Access-Control-Allow-Origin', '*')
            return response
        ...
        for mbid in mbids:
            ...
            if 'error' in check:
                response = Response(json.dumps({
                    'code': 400,
                    'error':
                    'MBID {0} cannot be found in MusicBrainz'.format(mbid)
                }), 400, mimetype="application/json")
                response.headers.add('Access-Control-Allow-Origin', '*')
                return response
                ...
                response = Response(json.dumps({
                    'code': 400,
                    'error': '''Provided match type {provided} doesn\'t match \
                                type {mbidtype} of {mbid}'''.format(
                                    provided=matchtype,
                                    mbidtype=check['type'],
                                    mbid=mbid)
                }), 400, mimetype="application/json")
    ...
    if auto:
        if not user:
            response = Response(json.dumps({
                'code': 400,
                'error': 'Automatic matches must provide a name.'
            }), 400, mimetype="application/json")
            response.headers.add('Access-Control-Allow-Origin', '*')
            return response
        if not ip:
            try:
                forward = request.environ['HTTP_X_FORWARDED_FOR']
                ip = forward.split(',')[-1].strip()
            except KeyError:
    ...
    try:
        ...
        response = Response(json.dumps({
            'code': 500,
            'error':
            'An unknown error happened while pushing to elasticsearch.'

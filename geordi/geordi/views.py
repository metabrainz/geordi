...
from flask import (render_template, request,
                   redirect, url_for, flash, Response, g)
from flask.ext.login import (login_required, login_user,
                             logout_user, current_user)
from geordi import app, login_manager, User, es
from geordi.search import (do_search, do_search_raw,
                           do_subitem_search, make_filters)
from geordi.matching import register_match, check_type
from geordi.mappings import (map_search_data, update_map_by_index,
                             update_linked_by_index, get_link_types_by_index,
                             update_automatic_item_matches_by_index,
                             update_automatic_subitem_matches_by_index,
                             get_mapoptions, get_code_url_by_index)
...
def before_request():
    ...
    g.link_types = dict([(index, get_link_types_by_index(index))
        ...
        in data['_source']['_geordi']['links']['links'].iteritems():
            if link_type != 'version':
                for link in links:
                    ...
                    subitems[subitem] = get_subitem(itemindex, subitem,
                                                    create=True,
                                                    seed=copy.deepcopy(link))
        return render_template(template,
                               item=item, index=itemindex, data=data,
                               mapping=data['_source']['_geordi']['mapping'],
                               mapoptions=mapoptions,
                               subitems=subitems, code_url=code_url)
...
def search():
    ...
    return render_template('search.html',
                           query=params.get('query'), data=params.get('data'),
            ...
            in document['_source']['_geordi']['links']['links'].iteritems():
                if link_type != 'version':
                    for link in links:
                        ...
                        get_subitem(index, subitem,
                                    create=True,
                                    seed=copy.deepcopy(link))
        response = Response(json.dumps({
            'code': 200, 'document': document
        }), 200, mimetype="application/json")
    except ElasticHttpNotFoundError:
        response = Response(json.dumps({
            'code': 404, 'error': 'The provided item could not be found.'
...
def matchitem(index, item):
    ...
    if current_user.is_authenticated():
        ...
        if request.args.get('unmatch', False):
            return register_match(index, item, 'item',
                                  'unmatch', [], auto, user)
    else:
        auto = True
        ...
        if user in ['matched by index']:
            return Response(json.dumps({
                'code': 400,
                'error': 'The name "{0}" is reserved.'.format(user)
            }), 400, mimetype="application/json")
...
def subitem(index, subitem):
    ...
    if subitem:
        response = Response(json.dumps({
            'code': 200, 'document': subitem
        }), 200, mimetype="application/json")
    else:
        response = Response(json.dumps({
            'code': 404,
            'error': 'The provided item could not be found.'
        }), 404, mimetype="application/json")
    response.headers.add('Access-Control-Allow-Origin', '*')
    return response
...
def matchsubitem(index, subitem):
    ...
    if current_user.is_authenticated():
        ...
        user = None
        if request.args.get('unmatch', False):
            return register_match(index, subitem, 'subitem',
                                  'unmatch', [], auto, user)
    else:
        auto = True
        user = request.args.get('user')
...
def internal_mbid_type(mbid):
    ...
    if 'error' in check:
        return Response(json.dumps({"error": "failed to fetch"}),
                        e.code,
...
def resolve_data(index, item):
    ...
    match_update = update_automatic_item_matches_by_index(index,
                                                          item,
                                                          data['_source'])
    ...
    if search_type == 'raw':
        ...
        if size is None and 'size' in json_query:
            try:
                ...
                pass
        try:
            data = do_search_raw(json_query, indices,
                                 start_from=request.args.get('from', None),
                                 filters=filters, doc_type=itemtypes,
                                 size=size)
        except ValueError:
            return {'error': "Malformed or missing JSON."}
    elif search_type == 'query':
        if query:
            data = do_search(query, indices,
                             start_from=request.args.get('from', None),
                             filters=filters, doc_type=itemtypes, size=size)
        else:
        ...
        data = do_subitem_search(query, index, subtype,
                                 start_from=request.args.get('from', None),
...
def flatten(l):
    for el in l:
        if isinstance(el,
                      collections.Iterable) and not isinstance(el, basestring):
            ...
            for key in seed.keys():
                if key not in data:
                    ...
                    try:
                        ...
                        changed = True
                    if ((isinstance(data[key], collections.Iterable) and
                         not isinstance(data[key], basestring))):
